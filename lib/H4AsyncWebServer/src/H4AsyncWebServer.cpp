...
void H4AsyncWebServer::route(void* c, const uint8_t* data, size_t len) {
    ...
    if (r->_blen) {  // refactor get
        ...
        if (r->_getHeader(_rqHeaders, "Content-type") == "application/x-www-form-urlencoded")
            r->_paramsFromstring(std::string((const char*)r->_body, r->_blen));
        else
            Serial.printf("received weird type %s\n", r->_getHeader(_rqHeaders, "Content-type").data());
    }
    //
    for (auto h : _handlers) {
        for (auto& s : h->_sniffHeader)
            if (_rqHeaders.count(uppercase(s.first))) h->_sniffHeader[s.first] = _rqHeaders[uppercase(s.first)];
